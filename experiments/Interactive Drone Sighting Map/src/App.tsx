    ...
    try {
      ...
      if (manualRefresh) {
        toast.error('Cannot load incidents', {
          description: 'Incident API unavailable due to CORS. Please configure CORS on your Lambda endpoint or connect Supabase.',
        });
      }
    } finally {
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <Button
              ...
              size="sm"
              disabled={isLoadingEvents || isLoadingIncidents}
            >
              <RefreshCw className={`w-4 h-4 mr-2 ${(isLoadingEvents || isLoadingIncidents) ? 'animate-spin' : ''}`} />
              {(isLoadingEvents || isLoadingIncidents) ? 'Loading...' : usingMockData ? 'Connect to API' : 'Refresh'}
            </Button>
      ...
      <div className="flex-1 flex overflow-hidden">
        <Tabs defaultValue="map" className="flex-1 flex flex-col">
          ...
          <TabsContent value="map" className="flex-1 m-0 relative">
            ...
            {(isLoadingEvents && events.length === 0) || (isLoadingIncidents && incidents.length === 0) ? (
